import { IPaginationExtra } from "index";
import { Knex } from "knex";


export default async function pagination<T>(queryObj: Knex.QueryBuilder, extra: IPaginationExtra): Promise<{ data: T[], extra: IPaginationExtra }> {
    const response: { data: T[], extra: IPaginationExtra } = {
        data: [], extra: {
    ...
    if (response.extra.getTotal) {
        if (response.extra.withGroup) {
            response.extra.total = (await knexInstance(queryObj.clone().clearSelect().clearOrder().count('* as total').as('grouped_sales')).count('* as total').first())?.total ?? 0;
        } else {
    ...
    if (extra.withOutData) {
        ...
        if (!extra.isAll) {
            ...
            // console.log( Number(response.extra.limit), Number(response.extra.page), 1)
